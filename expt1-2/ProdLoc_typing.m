% Function call: ProdLoc_typing(subjID, list, order, run)
%                eg. ProdLoc_typing('debug', 1, 1, 1)
%                for practice, pass 0 for list and 'practice' for run
%
% RUNTIME: 300 sec (5 min)
...
function ProdLoc_typing(subjID, list, order, run)
    ...
    if strcmp(run, 'practice')
        ...
        TRIAL_DUR      = 3;   %Length of trial
        STIM_DUR       = 2.8; %Length of stimulus presentation (rest of trial is +)
        PROD_TRIAL_DUR = 7;   %Length of trial in typing conditions (SPROD, WPROD, ARTIC)
        PROD_STIM_DUR  = 6.8;
        ITI            = 0;   %Inter-trial interval
    end
    ...
    %stimulus types
%     pic_rows  = strcmp(materials.Stim_Type, 'pic');        % SPROD, EVSEM
%     obj_set_rows = strcmp(materials.Stim_Type, 'obj-set'); % WPROD
%     sent_rows = strcmp(materials.Stim_Type, 'sent');       % SCOMP
%     w_list_rows = strcmp(materials.Stim_Type, 'w-list');   % WCOMP
%     nw_list_rows = strcmp(materials.Stim_Type, 'nw-list'); % ARTIC
% 
%     Pic    = materials(pic_rows, :);
    ...
    try
        for block = blocks
            ...
            for i=1:TRIALS_PER_BLOCK
                ...
                prod_conds = {'SPROD', 'WPROD', 'ARTIC'};
                if any(strcmp(prod_conds, block))
                    trialEndTime = onset + PROD_TRIAL_DUR + ITI;
                else
                ...
                PTBhelper('stimText', wPtr, '+', fixFontSize);
                if any(strcmp(prod_conds, block))
                    fixEndTime = trialEndTime - PROD_STIM_DUR;
                else
