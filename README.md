# Materials for Hu, Small, et al. (in prep)
...
The file [`experiment/ProdLoc_materials.csv`](experiment/ProdLoc_materials.csv) contains the list of materials expected by the MATLAB experimental script (see [Experimental script](#experimental-script) for more details). If you wish to alter the materials in order to re-run the experiment, then you can regenerate this file by running 
...
The actual experiment is launched using MATLAB with the scripts [`experiment/ProdLoc.m`](experiment/ProdLoc.m) and [`experiment/ProdLoc_typing.m`](experiment/ProdLoc_typing.m) for the spoken and typed experiments, respectively. Both scripts take the following arguments:
